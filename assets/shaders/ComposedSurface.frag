...
uniform vec2 u_sphere_rotation;
...
mat3 rotation_pitch(float angle)
{
    float c = cos(angle);
    float s = sin(angle);
    return mat3(
...
void main()
{
    ...
    if (u_sphere_projection_active != 0)
    {
        ...
        dir = rotation_yaw(u_sphere_rotation.x) * rotation_pitch(u_sphere_rotation.y) * dir;
