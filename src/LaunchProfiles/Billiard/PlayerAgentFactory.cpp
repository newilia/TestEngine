	...
	std::array<std::unique_ptr<IPlayerAgent>, 2> CreatePlayerAgents(
		...
		for (int playerIndex = 0; playerIndex < 2; ++playerIndex) {
			...
			case PlayerKind::LocalHuman:
				...
				agents[static_cast<std::size_t>(playerIndex)] = std::make_unique<LocalHumanPlayer>(

				    playerIndex, deps.ballInHandInput, deps.cueInput, deps.cueBallAimInput);

				break;

			case PlayerKind::RemoteHuman: {
				auto localDelegate = std::make_unique<LocalHumanPlayer>(
